  ...
  return (
    <DialogPrimitive.Portal>
      ...
      <DialogPrimitive.Content
        className={cn(
          'fixed left-1/2 top-1/2 z-50 w-[calc(100%-32px)] max-w-[480px] -translate-x-1/2 -translate-y-1/2',
          'rounded-lg border border-line-strong bg-panel p-5 shadow-pop data-[state=open]:animate-pop',
          className
        )}
        {...props}
      >
...
function DialogTitle({ className, ...props }: React.ComponentProps<typeof DialogPrimitive.Title>) {
  return (
    <DialogPrimitive.Title
      className={cn(
        'mb-3 text-micro font-bold uppercase tracking-[0.13em] text-ink2',
        className
      )}
      {...props}
    />
