...
- Report-Kopf (Runde 15): Gesamteinschätzung und Kurs sind `.head-card`-Karten (gleiche Höhe, Titel links unten ausgerichtet). Gesamt-Karte mit Verdict-Tönung (`g-pos`/`g-neg`/`g-neu`: getönter Rand + diagonaler Farbverlauf) und Ampel-Punkt; Kurs-Karte mit "Kurs"-Label und Pre-Market/Nachbörslich als abgetrenntem Bereich (border-top, Label links / Wert rechts). Report blendet gestaffelt ein (`#report > *` reportIn-Animation, prefers-reduced-motion beachtet). `.chart-aside`-Panels füllen die Chart-Höhe (flex).
