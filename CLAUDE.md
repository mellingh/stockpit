...
- Report-Kopf (Runde 15): Gesamteinschätzung und Kurs sind `.head-card`-Karten (gleiche Höhe, Titel links unten ausgerichtet). Gesamt-Karte mit Verdict-Tönung (`g-pos`/`g-neg`/`g-neu`: getönter Rand + diagonaler Farbverlauf), Ampel-Punkt und seit Runde 19 einer `.g-comps`-Zeile darunter (Technik/Analysten/News je mit Ampel-Punkt, Tooltip = Detailtext — füllt die Karte und bleibt keine Blackbox); Kurs-Karte mit "Kurs"-Label und Pre-Market/Nachbörslich als abgetrenntem Bereich (border-top, Label links / Wert rechts).
...
- **Fachbegriffe erklären** (Runde 19): Snowflake-Stärken/Risiken sind `{t, info}`-Objekte — `info` erklärt Abkürzungen (PEG, ROE, EPS, Free Cashflow, Short Float, Current Ratio …) laienverständlich und wird als eingerückte `.ov-punkt-info`-Zeile gerendert (Frontend akzeptiert auch plain Strings). Regel: In Bewertungs-/Fließtexten (Stärken, Risiken, News-Einordnung) Abkürzungen immer erklären; in reinen Kennzahlen-Tabellen nicht nötig. Report blendet gestaffelt ein (`#report > *` reportIn-Animation, prefers-reduced-motion beachtet). `.chart-aside`-Panels füllen die Chart-Höhe (flex).
